...
describe SncfApi::Request do
  ...
  describe '#fetch' do
    ...
    it 'should return a Response instance, whatever happen' do
      allow(Http).to receive_message_chain(:basic_auth, :get).and_return(Http::Response.new 200, nil, nil,nil)
      expect(@request.fetch('/coverage').class).to be(SncfApi::Response)
    end
